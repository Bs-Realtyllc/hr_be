...
exports.findByEmail = async (email) => {
  const [rows] = await db.query('SELECT * FROM employees WHERE email = ? LIMIT 1', [email]);
  return rows[0] || null;
};
...
exports.updatePasswordHash = async (id, hash) => {
  await db.query('UPDATE employees SET password_hash = ? WHERE id = ?', [hash, id]);
};
...
exports.create = async (data) => {
  const [result] = await db.query(
    `INSERT INTO employees
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [data.name, data.email, data.secondary_email, data.phone, data.emergency_contact, data.designation,
     data.department, data.manager_id, data.start_date, data.timezone, data.work_hours,
     JSON.stringify(data.tech_stack), data.role]
  );
...
exports.update = async (id, updates) => {
  ...
  const values = fields.map((f) => (f === 'tech_stack' ? JSON.stringify(updates[f]) : updates[f]));
  values.push(id);
...
exports.findProfilePictureById = async (id) => {
  const [rows] = await db.query('SELECT profile_picture FROM employees WHERE id = ?', [id]);
  return rows[0]?.profile_picture || null;
};

exports.updateProfilePicture = async (id, filename) => {
  await db.query('UPDATE employees SET profile_picture = ? WHERE id = ?', [filename, id]);
};
...
exports.findCitizenshipDocById = async (id, column) => {
  const [rows] = await db.query(`SELECT ${column} FROM employees WHERE id = ?`, [id]);

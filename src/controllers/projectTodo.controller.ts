...
export const create = asyncHandler(async (req: any, res: Response) => {
  ...
  const id = await repo.create(req.params.id, { title, description, deadline, sort_order }, req.user?.id ?? null);
  res.status(201).json({ id });
});

export const update = asyncHandler(async (req: any, res: Response) => {
  const { title, description, deadline, sort_order } = req.body;
  await repo.update(req.params.tid, { title, description, deadline, sort_order }, req.user?.id ?? null);
...
export const toggle = asyncHandler(async (req: any, res: Response) => {
  ...
  await repo.toggleComplete(req.params.tid, is_complete ? 1 : 0, req.user?.id ?? null);
  res.json({ success: true });
});

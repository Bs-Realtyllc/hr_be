...
export function withTaxEstimate(r: any) {
  ...
  const amount =
    r.amount != null ? Number(r.amount) : Math.round(monthlySalary);
  const taxAmount =
    r.tax_amount != null ? Number(r.tax_amount) : estimatedMonthlyTax;
  const taxPerc =
    r.tax_perc != null
      ? Number(r.tax_perc)
      : annualSalary > 0
        ? Number(((estimatedAnnualTax / annualSalary) * 100).toFixed(1))
        : 0;
...
export async function getTaxes(
  ...
  const rows = await employeeTaxRepo.findAllWithProfile(
    privileged ? null : userId,
    month,
    year,
  );
...
export async function updateTaxProfile(
  ...
  if (!panNo) {
    throw new AppError(
      "This employee has no panNo on file — set one before recording a tax profile",
      400,
    );
  }
  ...
  await employeeTaxRepo.upsertProfile(panNo, data.month, data.year, {
    amount,
    tax_amount,
    tax_perc,
  });
...
export async function buildSummary(privileged: boolean, userId: number) {
  ...
  return rows.map((r: any) => {
    ...
    return {
      ...
      net_pay: Math.round(
        baseMonthly + overtimePay + leaveDeduction + leaveBonus,
      ),
    };
  });
}
...
export async function runYearEndBonus(year: number) {
  ...
  for (const emp of employees as any[]) {
    ...
    const remainingDays = balances.reduce(
      (sum: number, b: any) => sum + Math.max(Number(b.remaining) || 0, 0),
      0,
    );
    if (remainingDays <= 0) continue;

    const monthlySalary = toMonthlySalary(emp.salary, emp.pay_frequency);
    const { dailyRate, amount } = calculateYearEndLeaveBonus(
      monthlySalary,
      remainingDays,
    );

...
| Projects | ✅ Done | `controllers/projects.js`, `routes/projects.js`, `models/Project.js`, `dtos/projectDto.js` → `project.controller.ts`, `routes/modules/project.routes.ts`, `models/Project.ts` + `ProjectAssignment.ts`/`Milestone.ts`/`ProjectService.ts`, `dtos/project.dto.ts`, `services/project.service.ts` | Milestones/assignments/services sub-resources included; those tables skip BaseModel (join/sub-resource tables, same rationale as departments/designations). Added validation to assignment/milestone/service sub-resource bodies where the old controller had none or partial checks. No auth middleware on this domain's routes — matches the original exactly. |
| Culture events | ✅ Done | `controllers/events.js`, `routes/events.js`, `models/CultureEvent.js`, `dtos/cultureEventDto.js` → `cultureEvent.controller.ts`, `routes/modules/cultureEvent.routes.ts`, `models/CultureEvent.ts`, `dtos/cultureEvent.dto.ts`, `services/cultureEvent.service.ts` | `cultureEventService.js` (used by the still-unconverted `profile.js` for the DOB → birthday-event sync) kept as-is, just repointed at `cultureEvent.repository.ts`. No auth middleware on this domain's routes — matches the original. |
| Holidays | ✅ Done | `controllers/holidays.js`, `routes/holidays.js`, `models/Holiday.js`, `dtos/holidayDto.js` → `holiday.controller.ts`, `routes/modules/holiday.routes.ts`, `models/Holiday.ts`, `dtos/holiday.dto.ts`, `services/holiday.service.ts` | Fully self-contained — no cross-cutting consumers to repoint. |
| Servers | ✅ Done | `controllers/servers.js`, `routes/servers.js`, `models/Server.js`, `dtos/serverDto.js` → `server.controller.ts`, `routes/modules/server.routes.ts`, `models/Server.ts`, `dtos/server.dto.ts`, `services/server.service.ts` | Fully self-contained. |
| Payroll | ✅ Done | `controllers/payroll.js`, `routes/payroll.js`, `models/PayrollAdjustment.js`, `models/EmployeeTax.js`, `dtos/payrollDto.js`, `services/payrollService.js` → `payroll.controller.ts`, `routes/modules/payroll.routes.ts`, `models/PayrollAdjustment.ts` (append-only ledger, skips BaseModel) + `models/EmployeeTax.ts`, `dtos/payroll.dto.ts`, `services/payroll.service.ts` | `payrollAdjustment.repository.ts` is used by `employee.service.ts` and `leave.service.ts` too — both repointed from `models/PayrollAdjustment`. |
| Overtime | ✅ Done | `controllers/overtime.js`, `routes/overtime.js`, `models/OvertimeRequest.js`, `dtos/overtimeDto.js`, `services/overtimeService.js` → `overtime.controller.ts`, `routes/modules/overtime.routes.ts`, `models/OvertimeRequest.ts`, `dtos/overtime.dto.ts`, `services/overtime.service.ts` | Approve flow books a payroll adjustment via `payrollAdjustment.repository.ts`. |
| Discord | ✅ Done | `controllers/discordWebhook.js`, `routes/discord.js` → `discordWebhook.controller.ts`, `routes/modules/discord.routes.ts` | Thin — reuses `discordStandupService.js` as-is (already repointed at the converted repositories during Standups/Employees). No new model/DTO/service needed. |
| Webhooks | ✅ Done | `controllers/webhook.js`, `routes/webhook.js` → `webhook.controller.ts`, `routes/modules/webhook.routes.ts` | **Bug fixed**: `month_end_report` was `async` but never wrapped in `asyncHandler`, and called `crypto.createHmac()` with an unchecked `HMAC_SECRET` outside any try/catch — confirmed this crashes the entire Node process (not just the request) when that env var is unset. Fixed with an explicit "not configured" check (same pattern `discordWebhook.controller.ts` already uses) + `asyncHandler`. `verify`/`receive` handlers kept but still unmounted (commented out), exactly as they were in the original — not this conversion's call to enable them. |
| Email settings | ✅ Done | `controllers/emailSettings.js`, `routes/emailSettings.js`, `models/EmailSettings.js`, `dtos/emailSettingsDto.js` → `emailSettings.controller.ts`, `routes/modules/emailSettings.routes.ts`, `models/EmailSettings.ts`, `dtos/emailSettings.dto.ts`, `services/emailSettings.service.ts` | `leave.service.ts`'s `sendEmailAsync` used to fetch SMTP config via `controllers/emailSettings.js`'s `getForSending` (a service importing from a controller) — repointed directly at `emailSettings.repository.ts`'s `findFullByEmployeeId`, removing that indirection. |
| Monthly reports | ✅ Done | `controllers/reports.js`, `routes/reports.js`, `models/MonthlyReport.js`, `dtos/monthlyReportDto.js` → `monthlyReport.controller.ts`, `routes/modules/monthlyReport.routes.ts`, `models/MonthlyReport.ts` (skips BaseModel — submission log, no update endpoint), `dtos/monthlyReport.dto.ts`, `services/monthlyReport.service.ts` | Multer disk storage path fixed to `path.join(process.cwd(), 'uploads', 'reports')` — a `__dirname`-relative path would have resolved under `dist/` once compiled (same class of bug the pilot's static-file serving had). Verified end-to-end with a real multipart upload + download. |
| Weekly reports | ✅ Done | `controllers/weeklyReports.js`, `routes/weeklyReports.js`, `models/WeeklyReport.js`, `dtos/weeklyReportDto.js` → `weeklyReport.controller.ts`, `routes/modules/weeklyReport.routes.ts`, `models/WeeklyReport.ts` (skips BaseModel, same reason), `dtos/weeklyReport.dto.ts`, `services/weeklyReport.service.ts` | Same `process.cwd()` fix applied to its per-week upload folders. `services/weeklyReminder.js` (the Sunday cron reminder) left untouched — it only reads `employee.repository.ts` already, no `weekly_reports` table access of its own. |
| Service credentials | ✅ Done | `controllers/serviceCredentials.js`, `routes/serviceCredentials.js`, `models/ServiceCredential.js`, `dtos/serviceCredentialDto.js` → `serviceCredential.controller.ts`, `routes/modules/serviceCredential.routes.ts`, `models/ServiceCredential.ts`, `dtos/serviceCredential.dto.ts`, `services/serviceCredential.service.ts` | `mergeParams: true` router preserved (mounted at `/service-credentials/:employeeId`). Verified the password-preserved-on-empty-update behavior directly against the DB. |
| Google / Meetings | ✅ Done | `controllers/googleController.js`, `controllers/meetings.js`, `routes/google.js`, `models/Meeting.js`, `models/GoogleSettings.js`, `dtos/meetingDto.js` → `google.controller.ts` + `meeting.controller.ts`, `routes/modules/google.routes.ts`, `models/GoogleSettings.ts` (singleton config, skips BaseModel) + `models/Meeting.ts`, `dtos/meeting.dto.ts`, `services/google.service.ts` + `services/meeting.service.ts` | `services/googleCalendar.js` and `services/googleEventShaper.js` (the actual OAuth/Calendar-API glue) deliberately kept as untouched `.js` — too risky to rewrite without a real OAuth flow to test against. `googleCalendar.js`'s five raw `db.query()` calls against `google_settings` (it bypassed the old model entirely) were repointed at the new `googleSettings.repository.ts` instead, so all access to that table now goes through one place. Verified read paths (`auth-url`, `status`, `meetings` list) against this environment's real, already-connected Google account and real calendar data — deliberately did **not** exercise the create-meeting write path further, to avoid creating a real calendar event on a live-connected account; confirmed no such event was created during testing. |
| Profile | ✅ Done | `controllers/profile.js`, `routes/profile.js`, `dtos/profileDto.js` → `profile.controller.ts`, `routes/modules/profile.routes.ts`, `dtos/profile.dto.ts`, `services/profile.service.ts` | Wraps the already-converted `employee.repository.ts`; no model of its own. `services/cultureEventService.js` kept as-is (birthday-event sync on DOB change). |
| Goals | ✅ Done | `controllers/goals.js`, `routes/goals.js`, `models/Goal.js`, `dtos/goalDto.js` → `goal.controller.ts`, `routes/modules/goal.routes.ts`, `models/Goal.ts`, `dtos/goal.dto.ts`, `services/goal.service.ts` | `goals.created_by` already served BaseModel's role; added `updated_by` only. The employee_id-resolution/authorization check in `create` runs in the controller, before DTO binding — it decides what employee_id the DTO even validates. |
| Performance | ✅ Done | `controllers/performance.js`, `routes/performance.js`, `models/PerformanceReview.js`, `dtos/performanceDto.js` → `performance.controller.ts`, `routes/modules/performance.routes.ts`, `models/PerformanceReview.ts`, `dtos/performance.dto.ts`, `services/performance.service.ts` | `reviewer_id` serves as the creator-audit field; added `updated_at`/`updated_by` only. Role checks that gate DTO validation (create) stay in the controller, same reasoning as Goals. |
| Feedback | ✅ Done | `controllers/feedback.js`, `routes/feedback.js`, `models/Feedback.js`, `dtos/feedbackDto.js` → `feedback.controller.ts`, `routes/modules/feedback.routes.ts`, `models/Feedback.ts` (immutable — create+delete only, skips BaseModel), `dtos/feedback.dto.ts`, `services/feedback.service.ts` | The "message required" and "no self-feedback" checks run in the controller before DTO binding, preserving the original's exact error precedence. |
| Policies | ✅ Done | `controllers/policies.js`, `controllers/policyAcknowledgements.js`, `routes/policies.js`, `models/Policy.js`, `models/PolicyAcknowledgement.js` → `policy.controller.ts` + `policyAcknowledgement.controller.ts`, `routes/modules/policy.routes.ts`, `models/Policy.ts` + `models/PolicyAcknowledgement.ts`, `dtos/policy.dto.ts`, `services/policy.service.ts` + `services/policyAcknowledgement.service.ts` | `services/policyAcknowledgementService.js` (the rejection-email notifier) kept as-is, repointed at `policy.repository.ts`. `employee.routes.ts`'s `/:id/acknowledgements` admin route (added during the Employees pilot) updated to import the new controller instead of the retired one. |
| Dashboard | ✅ Done | `controllers/dashboard.js`, `routes/dashboard.js`, `models/Dashboard.js` → `dashboard.controller.ts`, `routes/modules/dashboard.routes.ts`, `services/dashboard.service.ts`, `repositories/dashboard.repository.ts` | Converted last, as planned — pure aggregate reads over tables every other domain already modeled (`employees`, `leaveRequests`, `overtimeRequests`, `standups`, `projects`); no table/model of its own. |

## Conversion complete

All ~23 domains are now converted. `src/routes/index.js` is down to a single line mounting `src/routes/modules/index.ts`'s aggregate router — every domain lives there now. The pattern documented above (route → controller → service → repository → model, DTOs via `bindAndValidate`/`optionalNullable`, BaseModel audit columns added per-domain as needed) was applied consistently across all of them, each verified end-to-end against the real local dev database before moving to the next.
